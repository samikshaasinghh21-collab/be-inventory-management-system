...
import { formatInrCurrency, formatQuantity, toWholeQuantity } from "../../utils/formatters";
...
const ProjectManagementInventoryAllocation = () => {
  ...
  const changeItem = (index, key, value) => setForm((prev) => ({ ...prev, items: prev.items.map((item, i) => i === index ? { ...item, [key]: key === "requestedQty" && value !== "" ? toWholeQuantity(value) : value } : item) }));
  ...
  const save = async (submitAfter = false) => {
    ...
    try {
      const payload = { ...form, projectId: form.projectId, siteLocationId: form.siteLocationId, warehouseLocationId: form.warehouseLocationId, boqId: form.boqId || null, items: form.items.map((i) => ({ ...i, requestedQty: toWholeQuantity(i.requestedQty), name: i.name })) };
  ...
  const act = async () => {
    ...
    try {
      ...
      if (actionModal.action === "issue") await issueInventoryAllocation(actionModal.row.id, { ...payload, issueDate: today(), items: Object.entries(issueQuantities).map(([allocationItemId, quantity]) => ({ allocationItemId, quantity: toWholeQuantity(quantity) })).filter((item) => item.quantity > 0) });
  ...
  return <div className="space-y-5 pb-8">
    ...
    {actionModal&&<div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/40 p-4"><div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl"><h3 className="text-lg font-semibold capitalize">{actionModal.action} allocation</h3><p className="mt-1 text-sm text-slate-500">{actionModal.row.allocationNumber} · {projectMap.get(String(actionModal.row.projectId))?.name}</p>{actionModal.action==="issue"&&<div className="mt-4 space-y-2 rounded-xl border p-3">{actionModal.row.items.map(item=><label key={item.id} className="flex items-center justify-between gap-3 text-sm"><span><b className="block text-slate-800">{item.name}</b><span className="text-xs text-slate-500">Reserved balance {fmtQty(Math.max(q(item.approvedQty)-q(item.issuedQty),0))} {item.unit}</span></span><input type="number" min="0" max={Math.max(q(item.approvedQty)-q(item.issuedQty),0)} step="1" className="w-28 rounded-lg border px-2 py-2 text-right" value={issueQuantities[item.id]??0} onChange={e=>setIssueQuantities({...issueQuantities,[item.id]:e.target.value})}/></label>)}</div>}<textarea rows="4" className={`${inputClass} mt-4`} placeholder="Remarks" value={actionRemarks} onChange={e=>setActionRemarks(e.target.value)}/><div className="mt-4 flex justify-end gap-2"><button onClick={()=>setActionModal(null)} className="rounded-lg border px-4 py-2 text-sm font-semibold">Cancel</button><button disabled={busy} onClick={act} className={`rounded-lg px-4 py-2 text-sm font-semibold text-white ${actionModal.action==="reject"?"bg-rose-600":actionModal.action==="issue"?"bg-blue-600":"bg-emerald-600"}`}>{busy?"Working…":"Confirm"}</button></div></div></div>}
  </div>;
};
