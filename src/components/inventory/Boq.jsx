...
const Boq = () => {
  ...
  const loadRecords = () => {
    setRecords(getWorkflowList(STORAGE_KEY));
  };
  ...
  useEffect(() => {
    const handleChange = () => loadRecords();
    window.addEventListener(`${STORAGE_KEY}:changed`, handleChange);
    return () => window.removeEventListener(`${STORAGE_KEY}:changed`, handleChange);
  }, []);
